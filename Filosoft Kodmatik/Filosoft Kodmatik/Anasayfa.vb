...
Public Class Anasayfa
    ...
    Dim DBSecimi As String = Ayaryaz.ReadLine
    Dim AccesDBYolu As String = Ayaryaz.ReadLine
    ...
    Dim SqlDatabaseBaglan As New SqlConnection(DB_ConnectionString)
    Dim AccesDbBaglan As New OleDb.OleDbConnection("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" & AccesDBYolu)
    Dim SqlLiteDBBaglan As New SQLite.SQLiteConnection("Data Source=" & AccesDBYolu & ";Version=3;")
    ...
    Private Sub Kaydet_Buton_Click(sender As Object, e As EventArgs) Handles Kaydet_Buton.Click
        Try
            ...
            If Ms_Sql_Radio.Checked = True Then
                Ayaryaz.WriteLine("0")
            ElseIf Access_DB_Check.Checked = True Then
                Ayaryaz.WriteLine("1")
            Else
                Ayaryaz.WriteLine("2")
            End If

            Ayaryaz.Write(Access_DB_SqlLite_Text.Text)
            ...
            If SqlDatabaseBaglan.State = ConnectionState.Open Then
                SqlDatabaseBaglan.Close()
            End If
            ...
            If Access_DB_Check.Checked = True Then
                Dim AccessStr As String = ("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" & Access_DB_SqlLite_Text.Text)
                AccesDbBaglan = New OleDb.OleDbConnection(AccessStr)
                AccesDbBaglan.Open()
            ElseIf Ms_Sql_Radio.Checked = True Then
                Dim DB_ConnectionString As String = ("Server=" & Serveradı & ";Database=" & DatabaseAdı & ";MultipleActiveResultSets=True;Integrated Security=False ;User ID='" & KullanıcıAdı & "';Password='" & Sifre & "';Connect Timeout=3")
                SqlDatabaseBaglan = New SqlConnection(DB_ConnectionString)
                SqlDatabaseBaglan.Open()
            Else


                Dim DB_ConnectionString As String = ("Data Source=" & Access_DB_SqlLite_Text.Text & ";Version=3;")
                SqlLiteDBBaglan = New SQLiteConnection(DB_ConnectionString)
                SqlLiteDBBaglan.Open()
            End If
    ...
    Sub Ayar_Oku()
        ...
        If DBSecimi = "0" Then
            Ms_Sql_Radio.Checked = True
        ElseIf DBSecimi = 1 Then
            Access_DB_Check.Checked = True
        Else
            SQLite_Radio.Checked = True
        End If


        Access_DB_SqlLite_Text.Text = AccesDBYolu
    ...
    Private Sub insert_Buton_Click(sender As Object, e As EventArgs) Handles insert_Buton.Click
        ...
        Try
            ...
            If Ms_Sql_Radio.Checked = True Then
                If SqlDatabaseBaglan.State = ConnectionState.Closed Then
                    SqlDatabaseBaglan.Open()
                End If

                Dim Adapter As New SqlDataAdapter("Select * From " & Tablo_Adı_Text.Text & "", SqlDatabaseBaglan)
                Adapter.Fill(Datatablo)
            ElseIf Access_DB_Check.Checked = True Then
                If AccesDbBaglan.State = ConnectionState.Closed Then
                    AccesDbBaglan.Open()
                End If
                Dim Adapter As New OleDb.OleDbDataAdapter("Select * From " & Tablo_Adı_Text.Text & "", AccesDbBaglan)
                Adapter.Fill(Datatablo)
            Else

                If SqlLiteDBBaglan.State = ConnectionState.Closed Then
                    SqlLiteDBBaglan.Open()
                End If
                Dim Adapterx As New SQLite.SQLiteDataAdapter("Select * From " & Tablo_Adı_Text.Text & "", SqlLiteDBBaglan)
                Adapterx.Fill(Datatablo)
            End If
            ...
            Else
                ...
                Dim ix As Integer = 0
                If ilk_Sutunu_Dahil_Et_Check.Checked = False Then ix = 1



                For i = ix To TOPLAMSAYI - 1
                    KOD = KOD & "," & GRIDVIEW(i).ColumnName
                    INSERTKOD = INSERTKOD & ",@" & GRIDVIEW(i).ColumnName
                ...
                Dim Codex As String = "SqlCommand"
                If Access_DB_Check.Checked = True Then
                    Codex = "OledbCommand"
                End If

                If SQLite_Radio.Checked = True Then
                    Codex = "SQLiteCommand"
                End If

                KOD = "Using SqlKodu As New " & Codex & "(" & Chr(34) & "Insert Into " & Tablo_Adı_Text.Text & " (" & KOD & ") Values (" & INSERTKOD & ")" & Chr(34) & ",DatabaseBaglan)" & vbCrLf



                For s As Integer = ix To TOPLAMSAYI - 1
    ...
    Private Sub Update_Buton_Click(sender As Object, e As EventArgs) Handles Update_Buton.Click
        ...
        Try
            ...
            If Klavye_Radio.Checked = True Then
                ...
                My.Computer.Keyboard.SendKeys("Using Sqlkodu As New " & Codex & "({(}" & Chr(34) & "Update " & Tablo_Adı_Text.Text & " Set " & KOD & " Where ID='" & Chr(34) & "& ID &" & Chr(34) & "'" & Chr(34) & ",DatabaseBaglan{)}" & Chr(13))
            ...
            Else
                ...
                KOD = "Using Sqlkodu As New " & Codex & "(" & Chr(34) & "Update " & Tablo_Adı_Text.Text & " Set " & KOD & " Where ID='" & Chr(34) & "& Sırano &" & Chr(34) & "'" & Chr(34) & ",DatabaseBaglan)" & vbCrLf
    ...
    Private Sub Sutunları_Cek_Buton_Click(sender As Object, e As EventArgs) Handles Sutunları_Cek_Buton.Click
        ...
        If SqlDatabaseBaglan.State = ConnectionState.Open Then
            ...
            Dim TabloAdapter As New SqlDataAdapter("Select TABLE_NAME As Tablolar FROM INFORMATION_SCHEMA.TABLES Order  by TABLE_NAME Asc", SqlDatabaseBaglan)
    ...
    Private Sub Access_DB_Check_CheckedChanged(sender As Object, e As EventArgs) Handles Access_DB_Check.CheckedChanged

    End Sub

    Private Sub Firebird_Radio_CheckedChanged(sender As Object, e As EventArgs) Handles SQLite_Radio.CheckedChanged
